...
//=============================================================================
 /*:
 * @plugindesc v1.05 在地圖上點擊圖片運行事件  Causes common events to run when certain pictures
 ...
 * @desc 默認情況下是否啟用觸摸功能？NO - false YES - true默認值：true  Is touch movement enabled by default?
 * NO - false   YES - true   Default: true
 * @default true
 *
 * @param Hide Message
 * @desc 在消息期間隱藏有常見事件的圖片？ NO - false YES - true默認值：true  Hide Pictures with common events during messages?
 * NO - false   YES - true   Default: true
 * @default true
 *
 * @param ---圖片1 Picture 1 ---
 ...
 * @desc (點擊) 當具有此ID的圖片為時，運行此常見事件
 *點擊。對於沒有常見事件，將此值保留為0。Runs this common event when a picture with this ID is
 * clicked. Leave this value at 0 for no common event.
 * @default 0
 *
 * @param Picture 1 Repeat 
 * @desc (重複) 當保存具有此ID的圖片時運行此常見事件
 *每6幀。對於沒有常見事件，將此值保留為0。  Runs this common event when a picture with this ID is held
 * every 6 frames. Leave this value at 0 for no common event.
 * @default 0
 *
 * @param Picture 1 Hold   
 * @desc (保持) 當具有此ID的圖片為時，運行此常見事件
 *點擊並保持。對於沒有常見事件，將此值保留為0。  Runs this common event when a picture with this ID is
 * clicked and held. Leave this value at 0 for no common event.
 * @default 0
 *
 * @param Picture 1 Release
 * @desc (發布) 當具有此ID的圖片為時，運行此常見事件
 *發布。對於沒有常見事件，將此值保留為0。  Runs this common event when a picture with this ID is
 ...
 * Introduction
 * ============================================================================
 *圖片可以作為除了只是一個視覺的遊戲之外的東西。
  *他們有時可以作為UI元素或更多！ 但是，他們不是
  *有固有地做的能力是點擊公共事件時點擊
  *由鼠標。 使用這個插件，綁定到一個公共事件的圖片
  *點擊或觸摸時可用！
 ...
 * Instructions
 * ============================================================================
 *在插件的參數中，您可以為每個圖片ID設置公共事件ID
  *從1到100列出（RPG Maker MV中使用的最大圖片數量為100）。
  *如果圖片具有1個不透明度的值，則可以使用該圖片
  *為一個常見的事件。 有四種條件類型可以綁定
  *常見事件：
 ...
 * 點擊 - 
  *如果點擊圖片，這將提示常見事件觸發
  *如果列出的公共事件值高於0.這將只觸發一次
  *直到玩家釋放鼠標（或停止觸摸屏幕）。If the picture is clicked upon, this will prompt the common event to trigger
 ...
 * *如果單擊鼠標然後按住（或者按下觸摸屏）
  *保持），只要鼠標在圖片區域和公共事件上方
  *列出的值高於0，則這將每6幀觸發一次，直到
  *播放器釋放鼠標（或停止觸摸屏幕）或移動
  *鼠標在圖片邊界之外。If the mouse is clicked and then held (or if the touch screen is pushed and
...
  *如果單擊鼠標然後按住（或者按下觸摸屏）
  *保持），只要鼠標在圖片區域和公共事件上方
  *列出的值高於0，那麼這將觸發每一幀直到播放器
  *釋放鼠標（或停止觸摸屏幕）或將鼠標移到外面
  *的圖片邊界。If the mouse is clicked and then held (or if the touch screen is pushed and
 * held), as long as the mouse is above the picture area and the common event
...
  *釋放點擊的鼠標（或播放器停止觸摸的時刻）
  *屏幕），並釋放鼠標上面的圖片和如果圖片的
  *列出的公共事件值高於0，則在發佈時將觸發。Upon releasing the clicked mouse (or the moment the player stops touching
 ...
 *
 * ---
 * 請注意，對於綁定到的圖片，存在一些行為更改
  *常見事件的。 任何綁定到一個常見事件的圖片將是
  *與地圖的spriteset分離，而是綁定到場景。 這是
  *，以便在屏幕滾動，放大，搖動等情況下
  *不會影響裝訂的圖片。
  *：
  *這是因為圖片通常被用作UI元素和UI
 ...
 * ============================================================================
 *下面是一些插件命令，你可以使用，如果你想添加一個
  *圖像公共事件的控制位。
  *：
  *插件命令
  *：
  * EnableTouchMove    #開啟觸摸移動
  * DisableTouchMove   #關閉觸摸移動
  *
  * - 允許播放器按屏幕上的目的地並在那裡移動
  *禁止播放器使用觸摸移動。
  *：
  * HidePictureCommonEvents  #隱藏常見事件圖片
  * ShowPictureCommonEvents  #顯示常見事件圖片
  *
  * - 手動隱藏所有綁定到常見事件的圖片或顯示它們。 如果顯示的話
  *然後圖片將返回到之前的不透明度值。 隱藏的圖片
  *不能觸發激活公共事件。
 *======================================
 * 插件命令:
 * MovePlayer Down      #(滑鼠)下
 * MovePlayer Left      #(滑鼠)左
 * MovePlayer Right     #(滑鼠)右
 * MovePlayer Up        #(滑鼠)上
 *
 * - 將導致玩家角色移動，就像使用4 dir移動
 *輸入（通過鍵盤或控制器）。 這最好與“新聞”一起使用。
 ================================================
 * 插件命令:
 *   TriggerButton Ok        #(觸摸按鈕)確定
 *   TriggerButton Cancel    #(觸摸按鈕)取消
 *   TriggerButton Dash      #(觸摸按鈕)短跑
 *   TriggerButton PageUp    #(觸摸按鈕)上一頁
 *   TriggerButton PageDown  #(觸摸按鈕)下一頁
 *   TriggerButton Left      #(觸摸按鈕)左
 *   TriggerButton Up        #(觸摸按鈕)上
 *   TriggerButton Right     #(觸摸按鈕)右
 *   TriggerButton Down      #(觸摸按鈕)下
 *
 * - 這將導致遊戲模擬觸發按鈕命令
 *那些原始函數之一，即使有一個公共事件綁定到
 *該原始功能的所有鍵。
 =====================================================
